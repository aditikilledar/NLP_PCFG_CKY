# from https://github.com/lagmoellertim/pyCYK/blob/master/cyk.py
...
class CKY:
	...
	def __getCollectionSets(self, full_table, x_pos, x_offset):
        table_segment = []
        y_pos = 0

        while x_offset >= 2:
            item_set from tabulate import tabulate
...
class CYK:
    ...
    def __getCollectionSets(self, full_table, x_position, x_offset):
        ...
        print('v', vertical_combinations,  'h', horizontal_combinations, '\n')
    ...
    def __generateTable(self, word):
        ...
        for x_offset in range(2,len(word)+1):
            ...
            for x_position in range(len(word)):
                print('x_pos:', x_position, 'x_offset: ', x_offset)
    ...
    def outputTable(self, word):
        ...
        print(tabulate(pretty_table, list(word), tablefmt="grid"))= full_table[y_pos][x_pos:x_pos+x_offset]
            if x_offset > len(item_set):
                return None
        ...
        horizCombos = []
        for item in table_segment:
            vertCombos.append(item[0])
            horizCombos.append(item[-1])

        #return reverse of vertical combos, and also returns hori combos    
        return vertCombos[::-1], horizCombos

	def __getValidCombos(self, left_coll, right_coll):
		validCombos = []
